...
dirname=substring(dirname, 0, dirFileIndex);
...
N5posi=0;
neveropen=1;
for(iscanfolde=0; iscanfolde<list2D.length; iscanfolde++){
	
	jsonindex=lastIndexOf( list2D[iscanfolde],".json");
	...
	tifindex=lastIndexOf( list2D[iscanfolde],".tif");
	if(tifindex!=-1 && neveropen==1){
		open(dir+list2D[iscanfolde]);
		bitd=bitDepth();
		close();
		if(bitd==8)
		EightConv=false;
...
if(N5posi==1){
	run("N5 SliceSaver", "input="+dir+"/ output="+EightDir+" export=8bit_PackBits_tiff x="+xpad+" y="+ypad+" z="+zpad+" thread=12 lowest="+Minthre+" highest="+Maxthre+"");
...
if(EightConv==true){
	...
	list = getFileList(dir);
	Array.sort(list);
}else if (EightConv==false && N5posi==0){
	
	list = getFileList(dir);
}else if (EightConv==false && N5posi==1){
	dir=EightDir;
	list = getFileList(dir);
	Array.sort(list);
}
